...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.querySelectorAll(".tab").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    btn.classList.add("active"); ACTIVE_FILTER=btn.dataset.filter; render();
  }));
...
function render(){updateCards();renderTodo();renderTable();}
function updateCards(){
  const pending=TRIPS.filter(isPending).length, approved=TRIPS.filter(isApproved).length;
  const settle=TRIPS.filter(isSettleWait).length, settled=TRIPS.filter(isSettled).length;
  set("totalCount",TRIPS.length);set("pendingCount",pending);set("approvedCount",approved);
  set("settleCount",settle);set("settledCount",settled);set("tabPending",pending);set("tabSettle",settle);
}
function renderTodo(){
  ...
  if(!items.length){box.innerHTML='<div class="empty">현재 처리할 출장업무가 없습니다.</div>';return;}
  box.innerHTML=items.map(({t,type})=>{
    const p=period(t), text=type==="approve"?"승인 필요":"정산 필요";
    const action=type==="approve"
      ?`<button class="todo-action" onclick="location.href='trip-admin.html'">승인하기</button>`
      :`<button class="todo-action" onclick='openSettle(${JSON.stringify(t.id)})'>정산입력</button>`;
    return `<div class="todo"><div class="todo-main"><strong>${esc(t.userName)} · ${text}</strong><span>${esc(p)} · ${esc(t.destination||"-")}</span></div>${action}</div>`;
  }).join("");
}
...
function renderTable(){
  const b=document.getElementById("tripTable"),list=filteredTrips();b.innerHTML="";
  if(!list.length){b.innerHTML='<tr><td colspan="9" class="empty">조건에 맞는 출장내역이 없습니다.</td></tr>';return;}
  list.forEach(t=>{
...
function period(t){const s=t.tripStartDate||t.tripDate||"",e=t.tripEndDate||t.tripDate||s;return s===e?s:`${s} ~ ${e}`;}
function openSettle(id){
  const t=TRIPS.find(x=>x.id===id);if(!t)return;SETTLE_ID=id;
  document.getElementById("settleInfo").innerHTML=`<strong>${esc(t.userName)}</strong><br>${esc(period(t))} · ${esc(t.destination||"-")}`;
  document.getElementById("fuel").value=Number(t.fuelCost||0);document.getElementById("distance").value=Number(t.distance||0);
...
async function saveSettlement(){
  if(!SETTLE_ID)return;
  const btn=document.getElementById("settleSave"),body={action:"settleTrip",tripId:SETTLE_ID,fuelCost:Number(document.getElementById("fuel").value||0),distance:Number(document.getElementById("distance").value||0)};
  ...
  try{
    ...
    closeSettle();await loadTrips();alert("출장 정산이 완료되었습니다.");
  }catch(e){alert("서버 연결 실패");}
  finally{btn.disabled=false;btn.textContent="정산 완료";}
}
